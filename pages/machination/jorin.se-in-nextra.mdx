...
enumeration of the files and parsing their front-matter myself. I'll not be
implementing a `/[collection]/tags`. Maybe as a redirect to the *Tags* tab on
`/collection` just to keep the hierarchy properly navigable.
...
should function without JS. The packaged JS appears to be ~200kB per page
though, which is 
