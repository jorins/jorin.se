...
function json(i: any) { return JSON.stringify(i, undefined, 2) }
...
export function nest(headings: Heading[], targetDepth = 2): NestedHeadings {
  ...
  for (let index = 0; index < headings.length;) {
    ...
    if (targetDepth > item.depth) {
      ...
      throw new Error(`Heading nesting algorithm error: target depth ${targetDepth} exceeded received item's depth ${json(item)}`)
      ...
      // target depth.
      let subChunkEndIndex = headings.findIndex((heading, nextIndex, {length}) => {
